...
export default function ConfirmAlert({}) {
  ...
  const isDisplayedCautionModal = confirmState.show ? (
    <div className="fixed inset-0 z-40 flex items-center justify-center overflow-y-auto overflow-x-hidden bg-darkBlue3/0 outline-none backdrop-blur-sm focus:outline-none">
      <div className="flex w-269px flex-col items-center justify-center gap-4 rounded-[10px] bg-mermerTheme p-4 pt-5 font-Dosis text-sm text-lightWhite1">
        <div className="flex flex-col gap-10px">
          <div className="flex items-center justify-center gap-2">
            <Image src="/elements/caution.svg" width={24} height={24} alt="Caution icon" />
            <span className="text-xl font-bold">Caution</span>
          </div>
          <div>{confirmState?.text && confirmState.text}</div>
        </div>
        <div className="flex gap-2">
          ...
          >
            <span className="font-bold text-lightWhite1">Cancel</span>
          </button>
          <button
            onClick={onConfirm}
            className="flex h-auto w-110px items-center justify-center rounded-[5px] bg-lightBlue1 px-7px py-6px"
          >
            <span className="font-bold text-darkBlue2">Yes</span>
          </button>
        </div>
      </div>
    </div>
