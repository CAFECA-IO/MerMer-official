...
export default function TagsInputField({tags, setTags}: Props) {
  ...
  const filterTags = async (inputValue: string): Promise<IKmTag[]> => {
    ...
    return allTags.filter(i => i.label.toLowerCase().includes(inputValue.toLowerCase()));
  };
  ...
  return (
    <AsyncCreatableSelect
      ...
      classNames={{
        control: () =>
          'h-12 w-full  border-b-[1px] border-lightWhite1 bg-lightGray1/0 px-4 py-3 text-lightWhite1 outline-none',
        placeholder: () => 'text-lightGray1',
        valueContainer: () => 'gap-2 mb-3',
        multiValue: () => `text-darkBlue3 bg-lightBlue1 rounded-full text-sm font-Dosis px-2 py-1`,
        multiValueRemove: () => 'hover:text-red-500',
        menu: () =>
          'mt-2 py-1 bg-darkBlue3/50 backdrop-blur-lg rounded-md shadow-2xl border-[2px] border-lightGray2/50',
