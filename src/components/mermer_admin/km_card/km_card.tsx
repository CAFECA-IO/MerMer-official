...
export default function KmCard({
  kmId,
  published,
  title,
  date,
  ...
  return (
    <div
      ...
      data-tag={'key'}
      onClick={handleClick}
      className="relative flex h-166px w-full max-w-[944px] cursor-pointer items-center justify-start  gap-6 rounded-[10px] border-[3px] border-darkBlue3 bg-mermerTheme p-2 hover:border-lightBlue1/100"
    >
      <KmCardDropdown
      ...
      <div className="flex w-585px flex-col items-start justify-center gap-[11px] font-Dosis text-lightWhite1">
        ...
        <div className="flex w-full items-center justify-between text-base">
          <span>
            {[parsedDate.getFullYear(), parsedDate.getMonth() + 1, parsedDate.getDate()].join('/')}
          </span>
          <div
            className={`${isPublish ? 'flex' : 'hidden'} flex items-center justify-between gap-6`}
          >
