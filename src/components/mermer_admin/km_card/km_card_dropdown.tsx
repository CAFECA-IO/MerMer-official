...
export default function KmCardDropdown({
  kmId,
  kmTitle,
  isPublish,
  setIsPublish,
  ...
  const handlePublishUnpublishOnclick = (goPublish: boolean) => {
    ...
    const goPublishOrInpublish = async (event: React.MouseEvent) => {
      ...
      if (goPublish) {
        ...
        const allPublishedKmMeta = kmAllMeta.published.kmMetas
          ? [newPublishedKmMeta, ...kmAllMeta.published.kmMetas]
          : [newPublishedKmMeta];
        const updatePublishKmMeta = allPublishedKmMeta.filter(km => km !== undefined) as
          | IKmMeta[]
          | undefined;
        ...
        const allUnpublishedKmMeta = kmAllMeta.drafts.kmMetas
          ? [newUnpublishedKmMeta, ...kmAllMeta.drafts.kmMetas]
          : [newUnpublishedKmMeta];
        const updateUnpublishKmMeta = allUnpublishedKmMeta.filter(km => km !== undefined) as
          | IKmMeta[]
          | undefined;
        ...
        <div className="absolute -left-96px flex flex-col items-center justify-center rounded-[5px] bg-mermerTheme shadow-drop">
          ...
          >
            <div className="flex  items-center justify-center gap-2">
              <Image src="/elements/publish.svg" height={24} width={24} alt="Publish icon" />
          ...
          >
            <div className="flex items-center justify-center gap-2">
              <Image src="/elements/unpublished.svg" height={24} width={24} alt="unpublish icon" />
              <span className="text-base">Unpublish</span>
            </div>
          </MerMerDropdownButton>
          <MerMerDropdownButton
